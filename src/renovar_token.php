...
// ✅ Ruta segura al archivo token.json en Render
$tokenFile = sys_get_temp_dir() . '/token.json';

// 🔧 Crear token.json vacío si no existe
if (!file_exists($tokenFile)) {
    $handle = fopen($tokenFile, 'w');
    if ($handle) {
        fclose($handle);
    } else {
        die("❌ No se pudo crear el archivo token.json");
    }
}

// 📥 Leer el contenido actual del token
